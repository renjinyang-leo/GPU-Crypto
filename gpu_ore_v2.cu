...
//设置数据块长度16字节（128位）
#define INT64_KEY_LEN 9
#define INT64_BUF_LEN 18

typedef unsigned char byte;

typedef struct {
    byte buf[INT64_BUF_LEN];
} ciphertext_int64;

typedef int64_t int64;


static bool init_gpu_env = false;
...
__device__ void aes256_encrypt_ecb(uint8_t *buf_segment) {
    uint8_t buf_t[16]; // thread buffer
...
uint64_t reverse_bit64(uint64_t x) {
...
__global__ void general_order_revealing_encryption_gpu(uint8_t *buf_device_plaintext, uint32_t *buf_device_ciphertext, bool positive) {
    const int MASK_SIZE = 2;
    /**
     * 每一次截取明文的前bits位处理，并根据ECB规则将其补齐为16字节，补位的位填充0
     */
    unsigned int bits = threadIdx.x + 1;
    if (bits > INT64_KEY_LEN * 8) {
        return;
    }
...
int64_to_ciphertext_internal_v2(int64 value) {
    ...
    ciphertext_int64 *ctxt = (ciphertext_int64 *) malloc(sizeof(ciphertext_int64));

    // GPU显存中的明文和密文存储空间
    uint8_t *buf_device_plaintext = nullptr;
    ...
    else {positive = false; value = -value;}
    uint64_t u_value = (uint64_t)value;
    uint8_t cnt = 0;
    uint64_t temp_value = u_value;
    while (cnt <= 20) {       //uint64_t可表示最大值不超过 1e20
        if (temp_value >= 10) {temp_value = temp_value / 10;cnt++;}
        else {break;}
    }
    if (positive) { cnt = cnt | (1 << 7); }

    if (__BYTE_ORDER == __LITTLE_ENDIAN) { // 小端序, eg. 4 = ‘001’，需要翻转为大端序x
