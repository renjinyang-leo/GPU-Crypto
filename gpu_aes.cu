...
//aes加密
void encryptdemo(AesKey aes_key, uint8_t *buf, unsigned long numbytes){
  uint8_t *buf_device;
  uint8_t *key_device;
  ...
  //为数据和扩展后的密钥分配显存空间
  cudaMalloc((void**)&buf_device, numbytes);
  cudaMalloc((void**)&key_device, AES_KEY_SIZE);
  //从内存拷贝至显存
  cudaMemcpy(buf_device, buf, numbytes, cudaMemcpyHostToDevice);
  cudaMemcpy(key_device, aes_key.key, AES_KEY_SIZE, cudaMemcpyHostToDevice);
  ...
  //对每个数据块进行aes加密
  aes256_encrypt_ecb<<<dimBlock, dimGrid>>>(buf_device, numbytes, key_device);

  cudaMemcpy(buf, buf_device, numbytes, cudaMemcpyDeviceToHost);
}
...
int main(int argc,char** argv){
  uint8_t *buf, *pinned_buf;
  int  i;
  int padding;
 
  // 生成密钥
  uint8_t seed[32];
  for (i = 0; i < sizeof(seed);i++) seed[i] = i;
  AesKey key;
  create_aes_key(seed, key);
  ...
  unsigned long numbytes = sizeof(uint64_t);
  uint64_t plaintext = 10000000;
  buf = (uint8_t*)malloc(numbytes);
  memcpy(buf, &plaintext, numbytes);

  // 补全
  padding = AES_BLOCK_SIZE - numbytes % AES_BLOCK_SIZE;
  numbytes += padding;

  cudaMallocHost((void**)&pinned_buf, numbytes);
  cudaMemcpy(pinned_buf, buf, numbytes, cudaMemcpyHostToHost);
  ...
  // 调用加密算法
  encryptdemo(key, pinned_buf, numbytes);
  printf("ciphertext: ");
  for (int i = 0; i < numbytes; i++) {
    printf("%X", pinned_buf[i]);
  }
  printf("\n");
