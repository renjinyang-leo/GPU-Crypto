...
u32 Byte2Word(BYTE k1, BYTE k2, BYTE k3, BYTE k4){
...
void aes256_encrypt_ecb(uint8_t *buf_segment) {
...
void general_order_revealing_encryption_cpu(uint8_t *buf_host_plaintext, unsigned int *buf_host_ciphertext) {
    uint8_t buf_segment[SEGMENT_SIZE] = {0};
    for (int bits = 1; bits <= PLAINTEXT_SIZE * 8; bits++) {
        memset(buf_segment, 0, SEGMENT_SIZE);
        uint8_t target_bit = 0;
        memcpy(buf_segment, buf_host_plaintext, bits/8);
        
        if (bits % 8 != 0) {
            unsigned int last_index = bits / 8;
            uint8_t additional_bits = bits % 8;
            buf_segment[last_index] = buf_host_plaintext[last_index] & (uint8_t)(1<<additional_bits-1);
        ...
        * **/
        ...
        unsigned long cipher_index = ((bits-1) * 2) / 32;
        unsigned long cipher_bit = ((bits-1) * 2) % 32;
        //printf("cipher_index = %ld, cipher_bit = %ld, mask = %d, target_bit = %d\n", cipher_index, cipher_bit, mask, target_bit);
        buf_host_ciphertext[cipher_index] += ((unsigned int)mask)<<cipher_bit;
    }
}
...
void encyrpt(uint64_t plaintext) {
    ...
    memset(buf_host_ciphertext, 0, PLAINTEXT_SIZE * MASK_SIZE);

    general_order_revealing_encryption_cpu(buf_host_plaintext, buf_host_ciphertext);
...
void init_env() {
    ...
    for (int i = 0; i < 240; i += 4) {
        keywords[i/4]=Byte2Word(aes_key.key[i],aes_key.key[i+1],aes_key.key[i+2],aes_key.key[i+3]);
    }
    free(aes_key.key);

    buf_host_plaintext = (uint8_t *)malloc(PLAINTEXT_SIZE);
    buf_host_ciphertext = (unsigned int *)malloc(PLAINTEXT_SIZE * MASK_SIZE);
}

void free_env() {
    free(buf_host_plaintext);
    free(buf_host_ciphertext);
}
