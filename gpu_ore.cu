...
#define CHECK(x) if (x != cudaSuccess) {printf("Error: %s\n", cudaGetErrorString(x));printf("Exiting...\n");exit(EXIT_FAILURE);}
...
u32 Byte2Word_host(BYTE k1, BYTE k2, BYTE k3, BYTE k4){
    u32 result(0x00000000);  
    u32 temp;  
    temp = k1;  // K1  
    temp <<= 24;  
...
__constant__  u32 keywords[60];


__device__ void aes256_encrypt_ecb(uint8_t *buf_segment) {
    ...
    buf_t[0] = buf_segment[0];
    buf_t[1] = buf_segment[4];
    buf_t[2] = buf_segment[8];
    buf_t[3] = buf_segment[12];
    buf_t[4] = buf_segment[1];
    buf_t[5] = buf_segment[5];
    buf_t[6] = buf_segment[9];
    buf_t[7] = buf_segment[13];
    buf_t[8] = buf_segment[2];
    buf_t[9] = buf_segment[6];
    buf_t[10] = buf_segment[10];
    buf_t[11] = buf_segment[14];
    buf_t[12] = buf_segment[3];
    buf_t[13] = buf_segment[7];
    buf_t[14] = buf_segment[11];
    buf_t[15] = buf_segment[15];
    ...
    buf_segment[0]=(s0>>24)&0xff;
    buf_segment[1]=(s1>>24)&0xff;
    buf_segment[2]=(s2>>24)&0xff;
    buf_segment[3]=(s3>>24)&0xff;
    buf_segment[4]=(s0>>16)&0xff;
    buf_segment[5]=(s1>>16)&0xff;
    buf_segment[6]=(s2>>16)&0xff;
    buf_segment[7]=(s3>>16)&0xff;
    buf_segment[8]=(s0>>8)&0xff;
    buf_segment[9]=(s1>>8)&0xff;
    buf_segment[10]=(s2>>8)&0xff;
    buf_segment[11]=(s3>>8)&0xff;
    buf_segment[12]=s0&0xff;
    buf_segment[13]=s1&0xff;
    buf_segment[14]=s2&0xff;
    buf_segment[15]=s3&0xff;
}

__global__ void general_order_revealing_encryption_gpu(uint64_t *buf_device_plaintext, unsigned long index, unsigned int *buf_device_ciphertext) {
    /**
     * 每一次截取明文的前bits位处理，并根据ECB规则将其补齐为16字节，补位的位填充0
     */
    unsigned int bits = threadIdx.x + 1;
    if (bits > PLAINTEXT_SIZE * 8) {
        return;
    }
    ...
    /**
     * 对处理完的buf_segment取最后MASK_SIZE位，并写回记录结果的buf中
     * **/
    uint8_t mask = buf_segment[0] & ((uint8_t)(1 << MASK_SIZE) - 1);
    mask = (mask + target_bit) % 4;
    unsigned long cipher_index = (threadIdx.x * 2) / 32;
...
uint64_t *buf_device_plaintext = nullptr;
unsigned int *buf_device_ciphertext = nullptr;

uint64_t *buf_host_plaintext = nullptr;
unsigned int *buf_host_ciphertext = nullptr;

void load_data(unsigned int len) {
    CHECK(cudaMemcpy(buf_device_plaintext, buf_host_plaintext, PLAINTEXT_SIZE * len, cudaMemcpyHostToDevice));
}
...
void init_env(const unsigned len) {
    ...
    CHECK(cudaMemcpyToSymbol(cuda_Te0,Te0,256*sizeof(u32)));
    CHECK(cudaMemcpyToSymbol(cuda_Te1,Te1,256*sizeof(u32)));
    CHECK(cudaMemcpyToSymbol(cuda_Te2,Te2,256*sizeof(u32)));
    CHECK(cudaMemcpyToSymbol(cuda_Te3,Te3,256*sizeof(u32)));
    ...
    CHECK(cudaMemcpyToSymbol(keywords, keywords_host, sizeof(u32)*60));

    CHECK(cudaMalloc((void**)&buf_device_plaintext, PLAINTEXT_SIZE * len));
...
void free_env() {
    ...
    CHECK(cudaFreeHost(buf_host_plaintext));
    CHECK(cudaFreeHost(buf_host_ciphertext));
