...
__global__ void general_order_revealing_encryption_gpu(uint8_t *buf_device_plaintext, unsigned int *buf_device_ciphertext) {
    /**
    ...
    while (index <= SEGMENT_SIZE) {
        if (index * 8 <= bits) {
            buf_segment[index-1] = buf_device_plaintext[index-1];
        } else {
            buf_segment[index-1] = 0;
        }
        index++;
    }
    if (bits % 8 != 0) {
